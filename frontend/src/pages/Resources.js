...
import { resourceService, resourceTypeService } from '../services/api';
import { MapPin, Edit2, Trash2, PlusCircle, X, AlertCircle, Tag, Plus } from 'lucide-react';

const Resources = () => {
  const navigate = useNavigate();
  const [resources, setResources] = useState([]);
  const [resourceTypes, setCourtTypes] = useState([]);
  ...
  const loadData = async () => {
    try {
      await Promise.all([loadResources(), loadCourtTypes()]);
    } catch (error) {
  ...
  const loadCourtTypes = async () => {
    const response = await resourceTypeService.getAll();
  ...
  const validateForm = () => {
    ...
    if (!formData.resourceTypeId) {
      errors.resresourceTypeId = 'Tipo de quadra é obrigatório';
    }
  ...
  const openModal = (resource = null) => {
    if (resource) {
      setEditingCourt(resource);
      setFormData({
        name: resource.name,
        resourceTypeId: resource.resourceTypeId,
        pricePerHour: resource.pricePerHour.toString(),
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (editingCourt) {
        await resourceService.update(editingCourt.id, dataToSend);
  ...
  const handleDelete = async (id) => {
    ...
    const resourceName = resource ? resource.name : 'esta quadra';

    if (!window.confirm(`Tem certeza que deseja excluir a quadra "${resourceName}"?`)) {
      return;
    }
    ...
    try {
      ...
      if (error.response?.status === 409) {
        ...
        if (activeCount) {
          setError(
            `⚠️ Não é possível excluir a quadra "${resourceName}".\n\n` +
  ...
  const handleCreateType = async (e) => {
    ...
    try {
      await resourceTypeService.create({ name: typeFormData.name.trim() });
        ...
        {resources.length === 0 ? (
          ...
          <div className="grid grid-3" style={{ gap: '1.5rem' }}>
            {resources.map(resource => {
              ...
              return (
                <div key={resource.id} className="card hover-card">
                  ...
                  <div style={{ marginBottom: '1rem' }}>
                    <p className="text-muted" style={{ fontSize: '0.9rem', marginBottom: '0.5rem' }}>
                      <strong>Tipo:</strong> {resource.resourceType?.name}
                    </p>
                    <p style={{ fontSize: '0.9rem', marginBottom: '0.5rem', color: 'var(--primary)' }}>
                      <strong>Valor:</strong> R$ {resource.pricePerHour.toFixed(2)}/hora
                    </p>
            ...
            <div className="card" style={{ margin: 0, maxWidth: '600px', width: '100%' }}>
              ...
              <form onSubmit={handleSubmit}>
                ...
                <div className="grid grid-2" style={{ gap: '1rem' }}>
                  <div className="input-group">
                    <label htmlFor="resourceTypeId">Tipo de Quadra *</label>
                    <select
                      id="resourceTypeId"
                      name="resourceTypeId"
                      value={formData.resourceTypeId}
                      onChange={handleInputChange}
                      className={validationErrors.resourceTypeId ? 'input-error' : ''}
                    >
                    ...
                    {validationErrors.resourceTypeId && (
                      <span className="error-message">{validationErrors.resourceTypeId}</span>
                    )}
                  </div>
